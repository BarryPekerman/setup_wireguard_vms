...
The setup uses the official VPC module (v6) for correctness and maintainability, Ubuntu 22.04 LTS AMIs discovered dynamically per-region, and includes a NAT Gateway to allow the private instance secure outbound internet access (e.g., for package updates) without exposing it to inbound traffic. Region defaults to `eu-north-1`, and the bastion SSH ingress CIDR can be overridden via a variable or auto-detected locally using `curl`/`jq`.

## Requirements

- Terraform >= 1.0
- AWS account with credentials configured (e.g. `aws configure`) and permissions to manage VPC/EC2/SG/KeyPair
- SSH keypair (auto-generated by `setup.sh` as Ed25519 key)
  - Public key: `~/.ssh/wireguard-wireguard-setup.pub`
  - Private key: `~/.ssh/wireguard-wireguard-setup`
...
3. Connect using outputs:
   ```bash
   # Bastion
   ssh -i ~/.ssh/wireguard-wireguard-setup ubuntu@<bastion_public_ip>

   # Private via bastion
   ssh -i ~/.ssh/wireguard-wireguard-setup -o ProxyCommand='ssh -i ~/.ssh/wireguard-wireguard-setup -W %h:%p ubuntu@<bastion_public_ip>' ubuntu@<private_instance_ip>
   ```
